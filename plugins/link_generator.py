...
async def batch(client: Client, message: Message):
    while True:
        try:
            first_message = await client.ask(text = "<b>Forward the First Message from DB Channel ⏩ (with Quotes)..\n\nor Send the DB Channel Post Link\nUse /sbatch for stopping.</b>", chat_id = message.from_user.id, filters=(filters.forwarded | (filters.text & ~filters.forwarded)), timeout=60)
        ...
        if f_msg_id:
            ...
            await first_message.reply("❌ Error\n\nThis Forwarded Post is not from my DB Channel or This Link isn't taken from DB Channel", quote = True)
            continue
    while True:
        try:
...
async def link_generator(client: Client, message: Message):
    ...
    link = f"https://t.me/{client.username}?start={base64_string}"
    reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton(f"🔁 Share URL", url=f'{link}')]])
